...
func testRPC(t *testing.T, addr string) {
	...
	args = &Args{7, 8}
	mulReply := new(Reply)
	mulCall := client.Go("Arith", "Mul", args, mulReply, nil, false)
	addReply := new(Reply)
	addCall := client.Go("Arith", "Add", args, addReply, nil, false)
...
func (codec *CodecEmulator) Call(service string, method string, args *Args, reply *Reply) error {
...
func (codec *CodecEmulator) ReadRequestHeader(req *Request) error {
	req.Service = codec.service
	req.Method = codec.method
	req.Seq = 0
	return nil
}
...
func TestShutdown(t *testing.T) {
	...
	call := client.Go("Arith", "SleepMilli", args, reply, done, false)
	...
	for p := 0; p < procs; p++ {
		go func() {
			for atomic.AddInt32(&send, -1) >= 0 {
				gate <- true
				reply := new(Reply)
				client.Go("Arith", "Add", args, reply, res, false)
			}
		}()
		go func() {
			for call := range res {
